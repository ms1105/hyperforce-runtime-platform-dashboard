...
function renderKarpenterExecView(container) {
    ...
    const envAgg = {};
    // Get latest month from monthlyAgg (not trendData, as trendData may be filtered)
    const latestMonth = Object.values(monthlyAgg).length > 0
...
async function showClusterNodes(clusterName, month, clusterAvgCpu, environment) {
    ...
    // Use relative URL (same origin) - will be proxied by server.js
    // For GitLab Pages, use window.BINPACKING_API_URL if set, otherwise use same origin
    const apiUrl = window.BINPACKING_API_URL || '';
    let nodes = [];
    
    try {
        // Fetch node-level data from API
        const response = await fetch(`${apiUrl}/api/binpacking/nodes?cluster=${encodeURIComponent(clusterName)}&month=${targetMonth}`, {
            mode: 'cors',
        ...
        if (response.ok) {
            ...
            if (data.nodes && data.nodes.length > 0) {
                // Use actual node data from API
                nodes = data.nodes.map(node => ({
                    name: node.name,
                    avgCpu: node.avgCpu,
                    efficiencyIndicator: node.efficiencyIndicator,
                    efficiencyClass: node.efficiencyClass
                }));
            } else {
                // Fallback: create simulated nodes
                console.log('No node data returned, using simulated nodes');
                nodes = generateSimulatedNodes(clusterName, clusterAvgCpu, 10, environment);
            }
        } else {
            // Fallback: create simulated nodes
            console.log('API request failed, using simulated nodes');
            nodes = generateSimulatedNodes(clusterName, clusterAvgCpu, 10, environment);
        }
    } catch (error) {
